...
import player
from NoSongSelectionError import NoSongSelectionError
...
class View:
    def __init__(self, top=None):
        ...
        self.songProgress = ttk.Scale(top,from_=0,to=100,orient=tk.HORIZONTAL , value=0)
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo("success!", "db connected successfully...")
            else:
                raise Exception("Sorry! not connected to db")
        ...
        self.addFavourite.config(command=self.add_favourite)
        self.removeFavourite.config(command=self.remove_favourite)
        self.loadFavourite.config(command=self.load_favourite)
        self.isPaused=False
    ...
    def remove_favourite(self):
        sel_song_index=self.playList.curselection()

        try:
            if len(sel_song_index) == 0:
                raise NoSongSelectionError("please select a song...")
            song_name=self.playList.get(sel_song_index)
    ...
    def load_favourite(self):
        try:
            result=self.my_player.load_song_from_favourites()
            s=self.get_size_of_playList()
            if s==len(result[1]):
                raise NoSongSelectionError("No Song in your favourites")
            self.playList.delete(0,tk.END)
            for i in result[1]:
                self.playList.insert(tk.END,i)
            messagebox.showinfo("success","your song loaded...")
        except NoSongSelectionError as ex:
            messagebox.showerror("error",ex)
    ...
    def add_favourite(self):
        ...
        try:
            if len(sel_song_index)==0:
                raise NoSongSelectionError("please select a song...")
            song_name=self.playList.get(sel_song_index[0])
            ...
            messagebox.showerror("DB error!","some error in database. so please try again..")



    def get_size_of_playList(self):
        return len(self.playList.get(0,tk.END))


    def previous_song(self):
        #self.playList.selection_set(int(self.sel_song_index_tuple[0])-1)
        if self.song_index==0:
            self.playList.select_set(tk.END)
            self.playList.selection_clear(self.song_index)
            self.song_index = self.get_size_of_playList() - 1
        else:
            self.playList.select_set(self.song_index-1)
            self.playList.selection_clear(self.song_index)
            self.song_index = self.song_index - 1

    def closewindow(self):
        result=messagebox.askyesno("Close","Do you want to quit?")
    ...
    def pause_song(self):
        self.curr_sec = self.songProgress.get()
        if self.isPlay==True:
            ...
            self.my_player.play_song(self.curr_sec)
            self.setup_thread(self.curr_sec)
            self.isPlay=True
    ...
    def play_song(self):
        ...
        self.song_index=int(self.sel_song_index_tuple[0])
        try:
    ...
    def setup_thread(self,c):
        self.my_thread=threading.Thread(target=self.show_timer,args=(c,self.song_length))
        self.isPlaying=True
        self.isthreadrunning=True
        self.my_thread.start()

    def show_timer(self,c,total_sec):
        #curr_sec=1
        ...
        while self.curr_sec<=total_sec:
            ...
            time.sleep(1)
            if int(self.songProgress.get())==self.curr_sec:
                self.curr_sec=int(self.songProgress.get())+1
            else:
                self.curr_sec = int(self.songProgress.get()) + 1
                self.my_player.play_song(self.curr_sec)
            self.songProgress.config(value=self.curr_sec)
    ...
    def load_next_song(self):
        ...
        if self.next_song_index==self.get_size_of_playList():
    ...
    def remove_song(self):
        ...
        try:
            if len(self.song_index_tuple) == 0:
                ...
                song_name = self.playList.get(self.song_index_tuple)
    ...
    def show_song_details(self):
        ...
        self.songProgress.config(to=self.song_length,value=0)
    ...
    def add_song(self):
        ...
        for i in song_name:
            self.playList.insert(tk.END, i)
        color1 = hex(random.randint(0, 255))[2:]
        color2 = hex(random.randint(0, 255))[2:]
        color3 = hex(random.randint(0, 255))[2:]
        if len(color1) == 1:
            color1 = "0" + color1
        if len(color2) == 1:
            color2 = "0" + color2
